---
title: "Programming in C++ for R applications"
...
* Authors **R** packages [**bsvars**](https://cran.r-project.org/package=bsvars) and [**bsvarSIGNs**](https://cran.r-project.org/package=bsvarSIGNs) (more coming up)

![](bsvars.png){.absolute top=350 right=550 width="250"} 
![](bsvarSIGNs.png){.absolute top=350 right=250 width="250"}
...
* we're getting together to make things easier for each of us
* we're here to help each other
* commitment to learning from session to session is essential
* commitment to teaching others is essential
* start programming in **C++** for applications in **R** much faster
* share your experiences and advancements with others
* we're here to have fun and enjoy the process


## Motivations {background-color="#9933FF"}
...
## Objectives for the first meetings
...
## Materials for the first meetings

$$ $$

* Lecture slides
* **C++** scripts:
  * [`nicetry.cpp`](https://github.com/tw-presents/cpp4Rapp/blob/main/nicetry.cpp)
  * [`nicelr.cpp`](https://github.com/tw-presents/cpp4Rapp/blob/main/nicelr.cpp)
  * [`nicelist.cpp`](https://github.com/tw-presents/cpp4Rapp/blob/main/nicelist.cpp)
  * [`nicerig2.cpp`](https://github.com/tw-presents/cpp4Rapp/blob/main/nicerig2.cpp)
* **R** scripts:
  * [`nicepackage.R`](https://github.com/tw-presents/cpp4Rapp/blob/main/nicepackage.R)
...
## The first steps with Rcpp {background-color="#9933FF"}
...
$$
\widehat{Cov}\left[\widehat\beta\right] = \widehat\sigma^2 \left(X'X\right)^{-1}, \qquad \widehat\sigma^2 = \frac{1}{T} \left( Y - \widehat\beta X \right)'\left( Y - \widehat\beta X \right)
$$

* don't adjust the arguments
* return `beta_hat`, `sigma2_hat` and `cov_beta_hat` in a list
...
## Some stats with RcppArmadillo: random walk generator

::: {.callout-caution title="Your turn!"}
Write a function that generates a random walk with a given length `T` and a given standard deviation `sigma` of the innovations using **RcppArmadillo**.
...
1. adjust the arguments
2. return a list containing
...
3. Extend the `nicerig2.cpp` file with the **R** function that generates the random draws from the normal-incerted gamma 2 distribution.
4. Make certain the two functions generate exactly the same exactly the same draws.
5. Compare the two functions in terms of the computational speed using `bench::mark()`.
